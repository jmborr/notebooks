      PARAMETER (MA=4,MFIT=4,NCA=4,Nsz=3000)
      ...
      DIMENSION X1(Nsz),Y1(Nsz),Y3(Nsz),SIG1(Nsz),sum_t(Nsz)      
      character*80 arg(10)
      character*80 fplot
      EXTERNAL FJC
      fplot='~/bin/fplot'
      n_fit=2
 1    write(6,*)' dbfit2 f_name Qmin Qmax p1 p2 p3 p4'
      n_g=iargc()
      if(n_g.lt.7) then 
...
D	write(6,*)' Qmin and Qmax for fitting:',x(1),x(ndata)         
 ...
 100  ALAMDA=0
	...
	write(6,*)' parameter',i,a(i)
      ...
      write(fhistry,'(i2,4(1x,e14.6))')n_fit,(a(i),i=1,ma)
c      write(2,'(i5,3(2x,f6.2),2x,i2,2x,i2)') ndata,0.5,1.0,12.0,1,1
	 ...
	 y1(i)=yfit
      ...
      close(12)
      write(fname,'(a,a4)')filename(:lench(filename)),'FIT2'
      write(6,*)fname
      open(unit=11,file=fname)
      write(11,'(a)') fhistry
      write(11,'(a)') ftitle
      write(11,'(i5,3(2x,f6.2),2x,i2,2x,i2)') npts,0.5,Qmin,Qmax,1,0
      do i=1,npts
         write(11,*) x1(i),y3(i)
      enddo
      close(11)
...
C-------------------------------------------------------------
        ...
        REAL AC(*),DYDA(*)
	x2=x*x
        YFIT=AC(1)+(ac(2)+ac(3)*x2)*exp(-ac(4)*x2)
        DYDA(1)=1.
        DYDA(2)=exp(-ac(4)*x2)
        DYDA(3)=x2*exp(-ac(4)*x2)
	DYDA(4)=-(ac(2)+ac(3)*x2)*x2*exp(-ac(4)*x2)
        RETURN
        END
