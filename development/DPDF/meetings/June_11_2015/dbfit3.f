      ...
      fplot='~/bin/fplot'
      n_fit=3
 1    write(6,*)' dbfit3 f_name Qmin Qmax p1 p2 p3 p4'
      n_g=iargc()
      if(n_g.lt.7) then 
...
D	write(6,*)' Qmin and Qmax for fit:',x(1),x(ndata)         
...
c      write(2,'(i5,3(2x,f6.2),2x,i2,2x,i2)') ndata,0.5,1.0,12.0,1,1
	 ...
	 y1(i)=yfit
      ...
      close(12)
      write(fname,'(a,a4)')filename(:lench(filename)),'FIT3'
      write(6,*)fname
...
C-------------------------------------------------------------
        ...
        REAL AC(*),DYDA(*)
	x2=x*x
        YFIT=AC(1)+(ac(2)*x2+ac(3)*x2*x2)*exp(-ac(4)*x2)
        DYDA(1)=1.
        DYDA(2)=x2*exp(-ac(4)*x2)
        DYDA(3)=x2*x2*exp(-ac(4)*x2)
	DYDA(4)=-(ac(2)*x2+ac(3)*x2*x2)*x2*exp(-ac(4)*x2)
        RETURN
        END
